...
histGG<-function(cValue,sNum){
  require(ggplot2)
  workDir<-paste("depth",cValue,"/set",sNum,"/",sep = "")
  ...
  jpeg(filename = paste(workDir,'TreeOne',cValue,'.jpg',sep=""), width = 400, height = 300)
  ...
  jpeg(filename = paste(workDir,'TreeTwo',cValue,'.jpg',sep=""), width = 400, height = 300)
...
setwd("~/r/180108SmallRFDistance/data/rd2/")
...
for(i in cValue){
  for(j in sNum){
    ...
    treeDir<-paste("depth",i,"/set",j,"/Sp",i,".nex",sep = "")
    outDir<-paste("depth",i,"/set",j,"/",sep = "")
